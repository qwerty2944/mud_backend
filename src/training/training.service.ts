 ...
 * - 1회 +2 숙련도(상한 100). 값 조회→비용산정→골드차감→숙련도상승을 단일 트랜잭션 +
 *   유저별 advisory lock으로 직렬화해 동시요청 이중차감/경합을 차단한다.
 */
...
export class TrainingService {
  async train(userId: string, proficiencyType: string): Promise<TrainResult> {
    ...
    try {
      ...
      // 같은 유저의 shop/train 동시 실행 직렬화 (트랜잭션 종료 시 자동 해제)
      await client.query("select pg_advisory_xact_lock(hashtext($1))", [userId]);

      // 현재 숙련도 조회
      const { rows: pr } = await client.query<{ v: string | null }>(
      ...
      // 원자적 골드 차감 (음수 불가)
      const { rows: gr, rowCount } = await client.query<{ gold: number }>(
      ...
      const { rows: ur } = await client.query<{ new_value: string }>(
